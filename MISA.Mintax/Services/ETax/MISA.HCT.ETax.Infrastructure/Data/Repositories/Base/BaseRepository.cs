...
namespace MISA.HCT.ETax.Infrastructure.Data.Repositories
{
    public class BaseRepository<T> : IBaseRepository<T> where T : BaseEntity, IAggregateRoot
    {
        ...
        /// <param name="param"></param>
        /// <returns></returns>
        public Task<bool> CheckEntity(string procedureName, object[] param)
        {
            using (DatabaseConnector databaseConnector = new DatabaseConnector())
            {
                databaseConnector.SetParameterValue(procedureName, param);
                var value = databaseConnector.ExecuteScalar().Result;
        ...
        /// <param name="param"></param>
        /// <returns></returns>
        public Task<int> GetTotalEntity(string procedureName)
        {
            using (DatabaseConnector databaseConnector = new DatabaseConnector())
            {
                databaseConnector.SetParameterValue(procedureName, null);
                var value = databaseConnector.ExecuteScalar().Result;
