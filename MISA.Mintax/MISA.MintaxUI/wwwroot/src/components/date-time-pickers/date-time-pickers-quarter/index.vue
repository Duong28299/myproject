<template>
  <div>
    <VueDatePicker class="mintax-quater-picker" v-model="valueDate" :min-date="minDate" :max-date="maxDate" type="quarter" :locale="locale" @onChange="getValue" format="[Quý ]Q-YYYY" no-header :style="getWidth" color="#0E1D61" :placeholder="placeholder" />
  </div>
</template>
...
export default {
  ...
  methods: {
    getValue() {
      this.$emit("getValue", this.valueDate);
    },
  },
  computed: {
    getWidth() {
...
.vd-wrapper.mintax-quater-picker {
  &:focus,
  &:active {
...
.vd-picker__input .vd-icon.vd-icon {
  margin-right: 6px;
  margin-bottom: 0;
  color: $dark;
